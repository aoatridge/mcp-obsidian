...
export class GraphService {
    ...
    extractLinks(content) {
        ...
        while ((match = WIKILINK_PATTERN.exec(content)) !== null) {
            ...
            const link = { target: normalizedTarget };
            if (alias) {
                link.alias = alias;
            }
            links.push(link);
     ...
     */
    resolveLink(target, allNotes, sourceDir) {
        // Normalize target
        const normalizedTarget = target.endsWith('.md') ? target : `${target}.md`;
        // Direct match
     ...
     */
    async getGraphStats(hubCount = 10) {
    ...
    async getLocalGraph(params) {
        ...
        if (!centerNode) {
            centerNode = fullGraph.nodes.find(n => n.id.endsWith(normalizedPath) ||
        ...
        const includedNodes = new Set([centerNode.id]);
        const queue = [{ id: centerNode.id, currentDepth: 0 }];
        ...
        const localEdges = fullGraph.edges.filter(e => includedNodes.has(e.from) && includedNodes.has(e.to));
        return {
            nodes: localNodes,
            edges: localEdges
        };
